...
public class MCash extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actvCellNumber = (AutoCompleteTextView) findViewById(R.id.actvMobileNumbermCash);
        actvCellNumber.setAdapter(adapterContacts);

        editTextAmount = (EditText) findViewById(R.id.etAmountmCash);
        try
        {
        ...
        onClickButtonMcashMenuBackListener();
    }

    public void onClickButtonMcashMenuBackListener()
    {
        buttonMcashSend = (Button) findViewById(R.id.bSendNowmCash);
        buttonMcashSend.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        try {
                            int radioButtonID = radioGroupMCashPackage.getCheckedRadioButtonId();
                            View radioButton = radioGroupMCashPackage.findViewById(radioButtonID);
                            //package id 1 is Cashin and 2 is Cashout. index of prepaid is 0 and postpaid is 1
                            final int packageId = radioGroupMCashPackage.indexOfChild(radioButton)+1;
                            if(packageId == 1)
                            {
                                serviceId = Constants.SERVICE_TYPE_ID_MCASH_CASHIN;
                            }
                            else if(packageId == 2)
                            {
                                serviceId = Constants.SERVICE_TYPE_ID_MCASH_CASHOUT;
                            }
                            ...
                            final ProgressDialog progress = new ProgressDialog(MCash.this);
                            progress.setTitle("Processing");
                            progress.setMessage("Wait while executing MCash transaction...");
                            progress.show();
                            Thread bkashThread = new Thread() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        HttpPost post = new HttpPost(baseUrl + Constants.URL_TRANSACTION_MCASH);
