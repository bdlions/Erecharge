...
public class UCash extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actvCellNumber = (AutoCompleteTextView) findViewById(R.id.actvMobileNumberUCash);
        actvCellNumber.setAdapter(adapterContacts);

        editTextAmount = (EditText) findViewById(R.id.etAmountUCash);
        try
        {
        ...
        onClickButtonUcashMenuBackListener();
    }

    public void onClickButtonUcashMenuBackListener()
    {
        buttonUcashSend = (Button) findViewById(R.id.bSendNowUCash);
        buttonUcashSend.setOnClickListener(
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        try {
                            int radioButtonID = radioGroupUCashPackage.getCheckedRadioButtonId();
                            View radioButton = radioGroupUCashPackage.findViewById(radioButtonID);
                            //package id 1 is Cashin and 2 is Cashout. index of prepaid is 0 and postpaid is 1
                            final int packageId = radioGroupUCashPackage.indexOfChild(radioButton)+1;
                            if(packageId == 1)
                            {
                                serviceId = Constants.SERVICE_TYPE_ID_UCASH_CASHIN;
                            }
                            else if(packageId == 2)
                            {
                                serviceId = Constants.SERVICE_TYPE_ID_UCASH_CASHOUT;
                            }
                            ...
                            final ProgressDialog progress = new ProgressDialog(UCash.this);
                            progress.setTitle("Processing");
                            progress.setMessage("Wait while executing UCash transaction...");
                            progress.show();
                            Thread bkashThread = new Thread() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        HttpPost post = new HttpPost(baseUrl + Constants.URL_TRANSACTION_UCASH);
